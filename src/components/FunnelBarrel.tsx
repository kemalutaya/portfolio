 ...
 *
 * A WebGL drum of diagram cards (a spiral-template
 ...
 * clicking a card opens that diagram in the preview dialog via onOpen. A
 * visually-hidden button list keeps every card keyboard/SR accessible.
 */
...
/** The diagrams are 16:9 on this background; cards are 3:4, so each one is
 *  letterboxed onto a canvas in the diagram's own background colour. */
const CARD_BG = '#0B1720'
const CARD_W = 600
const CARD_H = 800
  ...
  useEffect(() => {
    ...
    function loadTex(url: string, matIndex: number) {
      ...
      img.onload = () => {
        ...
        if (!ctx) return fail()
        const ratio = img.naturalWidth && img.naturalHeight ? img.naturalHeight / img.naturalWidth : 9 / 16
        const h = CARD_W * ratio
        ctx.fillStyle = CARD_BG
        ctx.fillRect(0, 0, CARD_W, CARD_H)
        try {
          ctx.drawImage(img, 0, (CARD_H - h) / 2, CARD_W, h)
        } catch {
          return fail()
        }
        t.needsUpdate = true
  ...
  return (
    <div className="funnels__barrel" ref={wrapRef}>
      ...
      <span className="funnels__barrel-hint" aria-hidden="true">
        Drag to spin &middot; click a diagram to open
      </span>
