...
import { CalendarCheck, ShieldCheck, ClipboardText, CheckCircle } from '@/components/slab'

/**
 * IntroOverlay - "the workflow writes the line".
 *
 * The intro runs her daily process (schedule in, eligibility verified, chart
 * prepared, ready for the visit), drawn the way n8n draws a workflow: a half-pill
...
/** The four steps of the run: her daily chart-prep process, schedule to visit. */
const STEPS = [
  { label: 'Schedule in', trigger: true, Icon: CalendarCheck },
  { label: 'Eligibility verified', trigger: false, Icon: ShieldCheck },
  { label: 'Chart prepared', trigger: false, Icon: ClipboardText },
  { label: 'Ready for visit', trigger: false, Icon: CheckCircle },
] as const

const shouldRun =
  typeof window !== 'undefined' &&
  !window.matchMedia('(prefers-reduced-motion: reduce)').matches &&
  // Home is the deploy base (/portfolio/ on GitHub Pages), not the domain root.
...
export default function IntroOverlay() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (label) label.textContent = "Chart ready for tomorrow's visit"
      await wait(LOCK)
      if (cancelled) return
    ...
    <div className="boot" aria-hidden="true" role="presentation">
      ...
      <div className="boot__canvas" ref={canvasRef}>
        ...
        <span className="boot__status" ref={statusRef}>
          ...
          <span className="boot__status-text">Preparing tomorrow's charts</span>
        </span>
      </div>
    </div>
  )
