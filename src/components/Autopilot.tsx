 ...
 * Autopilot
 *
 * The physician-schedule macro from the automation case study, drawn as a
 * flow in a window frame: node cards (Excel sheet as the trigger, my review
 * as the orange-ringed gate), curved SVG bezier "cables" between them, and
 * glowing "signal" dots that travel ALONG each cable via GSAP MotionPathPlugin.
 * It is an illustration of the process, not a screenshot, and carries no
 * patient or facility data. Steps and outcomes come from the live site's
 * #improvements section.
 *
 * Nodes are absolutely positioned on a fixed 1000x388 design canvas; cables
 * are measured from the live node rects on mount + resize, so the wiring is
 * always correct. Reduced-motion drops the signals; offscreen pauses the tweens.
 */
...
// x,y = top-left of the node box on the design canvas.
// Two tiers: the macro's steps across the top, the errors it removed below.
const NODES: FlowNode[] = [
  // top chain: the schedule macro, step by step
  { id: 'n-sheet',  Icon: MicrosoftExcelLogo,   title: 'Excel sheet',   subtitle: 'Doctor + facility assignments',     x: 6,   y: 36, variant: 'trigger' },
  { id: 'n-match',  Icon: Crosshair,            title: 'Match',         subtitle: 'Highlighted facility to its doctor', x: 221, y: 36 },
  { id: 'n-attach', Icon: Paperclip,            title: 'Attach',        subtitle: 'Required files located',            x: 436, y: 36 },
  { id: 'n-draft',  Icon: MicrosoftOutlookLogo, title: 'Outlook draft', subtitle: 'Recipient, CC, subject, body, files', x: 651, y: 36 },
  { id: 'n-review', Icon: UserCheck,            title: 'My review',     subtitle: 'Every detail, then send',           x: 866, y: 36, variant: 'gate' },
  // outcomes: the three errors the macro eliminated
  { id: 'n-doctor', Icon: Stethoscope,          title: 'Right doctor',  subtitle: 'No wrong-doctor errors',            x: 226, y: 268, variant: 'win' },
  { id: 'n-cc',     Icon: UsersThree,           title: 'Right CC',      subtitle: 'No wrong-CC errors',                x: 546, y: 268, variant: 'win' },
  { id: 'n-files',  Icon: Files,                title: 'Right files',   subtitle: 'No wrong-attachment errors',        x: 866, y: 268, variant: 'win' },
]
...
const LINKS: Link[] = [
  // the macro's steps (solid)
  { from: 'n-sheet',  to: 'n-match' },
  { from: 'n-match',  to: 'n-attach' },
  { from: 'n-attach', to: 'n-draft' },
  { from: 'n-draft',  to: 'n-review' },
  // what a reviewed draft no longer gets wrong (dashed)
  { from: 'n-review', to: 'n-doctor', kind: 'dash' },
  { from: 'n-review', to: 'n-cc',     kind: 'dash' },
  { from: 'n-review', to: 'n-files',  kind: 'dash' },
]

export const TOOLS: { Icon: Icon; label: string }[] = [
  { Icon: MicrosoftExcelLogo,   label: 'Excel' },
  { Icon: MicrosoftOutlookLogo, label: 'Outlook' },
]
...
export default function Autopilot({ compact = false, maxScale = 1 }: AutopilotProps) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      LINKS.forEach((link, i) => {
        ...
        sig.setAttribute('filter', 'drop-shadow(0 0 5px rgba(25, 181, 165,.9))')
  ...
  return (
    <section
      ...
      id="autopilot"
      aria-label={compact ? 'How the physician-schedule macro works, step by step' : undefined}
      aria-labelledby={compact ? undefined : 'autopilot-heading'}
      data-reveal
    >
      {!compact && (
      <header className="autopilot__head">
        <span className="autopilot__eyebrow">Automation</span>
        <h2 id="autopilot-heading" className="autopilot__headline">
          How the schedule macro works.
        </h2>
        <p className="autopilot__intro">
          Excel holds the doctor and facility assignments. The macro matches the
          highlighted facility to the assigned doctor, attaches the required files
          and builds the Outlook draft. I review every detail, then send.
        </p>
      </header>
      )}
      ...
      <div className="autopilot__window">
        <div className="autopilot__titlebar" aria-hidden="true">
          ...
          <span className="autopilot__titlebar-label">Schedule macro: illustration</span>
        </div>
