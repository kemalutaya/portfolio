...
const frag = `
  ...
  void main(){
    ...
    vec3 bgLight   = vec3(0.961, 0.969, 0.965); // #F5F7F6 cream
    vec3 lineLight = vec3(0.400, 0.525, 0.506);  // soft neutral gray contour on cream
    vec3 bgDark    = vec3(0.051, 0.090, 0.118); // #081218 navy ink
    vec3 lineDark  = vec3(0.839, 0.949, 0.929);   // solid white on navy (black would be invisible)
...
export default function HeroCanvasV2() {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(darkMix.value ? 0x0d171e : 0xf5f7f6, 1)

    const onThemeChange = () => {
      const dark = getTheme() === 'dark'
      targetDark = dark ? 1 : 0
      renderer.setClearColor(dark ? 0x0d171e : 0xf5f7f6, 1)
    }
