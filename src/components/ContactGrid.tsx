...
import { PaperPlaneTilt, CheckCircle, WarningCircle, EnvelopeSimple, ArrowUpRight, CaretDown, CalendarBlank, Phone } from '@/components/slab'
import { FAQS } from '@/data/faqs'
import { profile } from '@/data/profile'
import { readLead, submitLead, SubmitError, ENDPOINT, MAX_NAME, MAX_EMAIL, MAX_MESSAGE, type SubmitResult } from '@/lib/contact'
...
/* Contact channels from the live site's Contact section. The booked call is
   the primary action; email, phone and LinkedIn are fallbacks. */
const BOOKING_URL = 'https://cal.com/kem-alutaya'
const PHONE = { label: '+63 950 454 3924', href: 'tel:+639504543924' }
    ...
    <section className="pgrid cgrid" aria-labelledby="contact-title">
      <header className="pgrid__head">
        ...
        <h1 className="pgrid__title" id="contact-title">
          Looking for a full-time healthcare operations role.
        </h1>
        <p className="pgrid__lede">
          I’m available immediately for a full-time remote role in insurance verification, EMR support, chart
          preparation or healthcare documentation, and open to contract work alongside that. Based in the
          Philippines, aligned to UK and U.S. business hours. Send me the role and I’ll tell you honestly whether
          I’m a fit.
        </p>
      </header>
      ...
      <div className="home__glass cgrid__glass">
        ...
        <aside className="cgrid__aside" aria-labelledby="contact-faq">
          ...
          <div className="cgrid__direct">
            <a className="cgrid__mail" href={BOOKING_URL} target="_blank" rel="noopener noreferrer">
            ...
            <a className="cgrid__mail" href={PHONE.href}>
          ...
          {status.kind === 'sent' ? (
            <div className="cgrid__done" role="status">
              ...
              <h2 className="cgrid__done-title">
                {status.via === 'webhook' ? 'Message sent.' : 'Your mail app has it.'}
              </h2>
              <p className="cgrid__done-body">
                {status.via === 'webhook'
                  ? 'Thanks for getting in touch. I’ll reply by email.'
                  : 'The message is laid out and addressed to me. Press send there and I’ll reply by email.'}
              </p>
            ...
            <form className={`cgrid__form${busy ? ' is-sending' : ''}`} onSubmit={onSubmit} noValidate>
              ...
              <label className="cgrid__field">
                ...
                <input type="email" name="email" autoComplete="email" required maxLength={MAX_EMAIL} placeholder="name@practice.com" />
              </label>

              <label className="cgrid__field cgrid__field--grow">
                <span className="cgrid__label">The role or the work</span>
                <textarea
                  name="message"
                  required
                  maxLength={MAX_MESSAGE}
                  placeholder="Practice or company, what you need help with, hours or timezone, and role type (full-time, part-time or contract)."
                />
              </label>
              ...
              <div className="cgrid__actions">
                ...
                {status.kind === 'error' ? (
                  ...
                  <span className="cgrid__hint">
                    {ENDPOINT ? 'Sends straight to my inbox.' : 'Opens your email app with the message filled in.'}{' '}
                    Prefer to talk?{' '}
